...
private fun downloadPdfFromBrowser(fileName: String, pdfBase64: String): Boolean =
    downloadPdfFromBrowserJs(fileName, pdfBase64).toString() == "true"

private fun downloadPdfFromBrowserJs(fileName: String, pdfBase64: String): JsAny? = js(
    """
    (() => {
      try {
        if (
...
private fun openPdfFromBrowser(fileName: String, pdfBase64: String): Boolean =
    openPdfFromBrowserJs(fileName, pdfBase64).toString() == "true"

private fun openPdfFromBrowserJs(fileName: String, pdfBase64: String): JsAny? = js(
    """
    (() => {
      try {
        if (
...
private fun printHtmlFromBrowser(title: String, html: String): Boolean =
    printHtmlFromBrowserJs(title, html).toString() == "true"

private fun printHtmlFromBrowserJs(title: String, html: String): JsAny? = js(
    """
    (() => {
      try {
